...
if($request == "checkFriend") {
    ...
    if(!DB::query('SELECT status FROM relationship WHERE user_one=:user_one AND user_two=:user_two',
        ...
        echo json_encode(0); // Not friends
    } else {
        ...
        if ($status == 0 && $viewerid == $actionUser) {
            echo json_encode(1); // Request sent from viewer
        } else if ($status == 0 && $viewerid != $actionUser) {
            echo json_encode(2); // Request sent from user
        } else if ($status == 1) {
            echo json_encode(3); // Friends
        }
    }
}
...
if($request == 'checkFollower') {
    ...
    if(DB::query('SELECT follower_id FROM followers WHERE user_id=:userid AND follower_id=:visitorid',
        array(':userid'=>$userid, ':visitorid'=>$viewerid))) {
        echo json_encode(true);
    } else {
        echo json_encode(false);
    }
